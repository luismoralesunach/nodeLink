...
type LinksContextType = {
  ...
  addLink: (linkInfo: Link) => Promise<void>
  removeLink: (id: string) => Promise<void>
  updateLink: (linkId: string | number, linkInfo: any) => Promise<void>
}
...
export function LinksProvider({ children }: { children: ReactNode }) {
  ...
  const removeLink = async (id: string) => {
   ...
   const updateLink = async (linkId:any, linkInfo: any) => {
    try {
      const { data } = await axios.put(`${API_URL}links/${linkId}/`, linkInfo)
      if(data){
  ...
  return (
    <LinksContext.Provider value={{ state, dispatch, addLink, removeLink, updateLink }}>
      {children}
