...
export function UserProvider({ children }: { children: ReactNode }) {
  const { state: linksState } = useLinks()
  const { links, dispatch: linksDispatch } = linksState
  ...
  const getProfile = async (profile: string) => {
    try {
      ...
      dispatch({ type: "SET_LINKS", payload: res.data.links }) // Set links in the LinksContext
  ...
  const updateProfile = async(profileInfo: any)=>{
    try {
      const response = await axios.put(`${API_URL}profile/${state.user.username}/update/`, profileInfo)
  ...
  return (
    <UserContext.Provider value={{ state, dispatch, login, logout, preview, setPreview, updateProfile }}>
      {children}
